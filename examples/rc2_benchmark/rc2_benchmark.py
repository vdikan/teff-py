...
def read_end_path_kpoint(fname):
    """Read the high-symmetry kpoint conventional label from the output
    of phonon dispersion calculation."""
    path_line = (grep["path", fname] | tail["-1"])()
...
class WfApp(cli.Application):
    ...
    def main(self):
        ...
        # system label obtained from the root path name.
        # It will be used for the index column label in the results dataframe.
        syslabel = pwd().strip().split("/")[-1]
        ...
        # cosmetics: create the label for the last column of the dataframe
        # so that it includes the label of the kpoint that should be - presumably -
        # at the edge of the Brillouin zone (end of the generated kpoints path). 
        # That conventional label is extracted from the output generated by the
        # `phonon_dispersion_relations`.
        edge_kpoint_label = read_end_path_kpoint(ph_disp.path+"/out.log")
        edge_column_name = "dfreq_rel_max_" + edge_kpoint_label
        ...
        df['stride'] = df['stride'].astype('int') # coerce `stride` column to integer
        # output and store the results dataframe
        print(df)
        df.to_csv('results.csv', index_label=syslabel)
        # display and save the chart (using `heatmap.py`)
        local["python"]("heatmap.py")
